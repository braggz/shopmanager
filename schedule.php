...
<?php if($auth>=3){
  echo "<form action = \"modifymills.php\" method = \"post\">";
} ?>
...
//echo $auth;
  ...
  $result = $mysqli -> query("SELECT id,millnum,employee, partnumber,task_date,comments,task FROM schedule ORDER BY id  DESC LIMIT 10 ");
...
echo"<th>ID</th>
    ...
    while($row = $result->fetch_assoc()){
      ...
      echo "<td style = \"border: 1px solid; padding:6px;\">".$row["millnum"]."</td>";
      echo "<td style = \"border: 1px solid;padding:6px;\">"."".$row["employee"]."</td>";
      echo "<td style = \"border: 1px solid;padding:6px;\"> "."".$row["partnumber"]."</td>";
      echo "<td style = \"border: 1px solid;padding:6px;\">"."".$row["task_date"]."</td>";
      echo "<td style = \"border: 1px solid;padding:6px;\">"."".$row["comments"]."</td>";
      echo "<td style = \"border: 1px solid;padding:6px;\">"."".$row["task"]."</td>";
      if($auth >=3 ){
    ...
    echo "</table>";
    if($auth >= 3){
      echo "<button type =\"submit\">Edit Schedule</button>";
      echo "<br>";
      echo "<button type = \"submit\" value = \"1\" name = \"deleteOrders\"> Delete Mill </button>";
      echo "</form>";
      echo "<form action = \"newmill.php\" method = \"post\">";
      echo "<button type =\"submit\">New Mill</button>";
      echo "</form>";
